...
class SearchReplacer {
  ...
  private $exact_match;
	 ...
	 */
	function __construct( $from, $to, $recurse_objects = false, $exact_match = false, $case_insensitive = false ) {
		$this->from = $from;
		$this->to = $to;
	...
	private function _run( $data, $serialised, $recursion_level = 0, &$visited_data = array() ) {
        ...
        if ($this->exact_match) {
					if ($this->case_insensitive) {
						if (strtolower($this->from) == strtolower($data) ) {
							$data = $this->to;
						}
					} else {
	          if ($this->from == $data) {
	            $data = $this->to;
					...
					if ($this->case_insensitive) {
						$data = str_ireplace( $this->from, $this->to, $data );
					} else {
