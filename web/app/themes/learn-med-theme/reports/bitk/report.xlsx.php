...
# set defaults
  ...
  $filename = 'Report - Mistreatment - ' . date('F j, Y') . '.xlsx';
    ...
    # store in $modules[module_activity_id][students]
      ...
      foreach($response as $learner) {
        ...
        if (isset($learner['completion']) && true == $learner['completion']) {
          ...
          $student_details['role'] = get_user_meta($wp_user_id, 'Role', true);
          $student_details['hospital'] = get_user_meta($wp_user_id, 'Hospital', true);
          $student_details['other_company'] = get_user_meta($wp_user_id, 'OtherCompany', true);
          ...
          $student_details['division'] = get_user_meta($wp_user_id, 'Division', true);
          $student_details['employeeID'] = get_user_meta($wp_user_id, 'adi_employeeid', true);
          $student_details['user_id'] = get_user_meta($wp_user_id, 'nickname', true); 
        }
...
# assemble the xls
  ...
  # add the column names
    ...
    $objWorksheet->setCellValue('E2', 'Role');
    $objWorksheet->setCellValue('F2', 'Company/Hospital');
    $objWorksheet->setCellValue('G2', '> Other');
    $objWorksheet->setCellValue('H2', 'Department');
    $objWorksheet->setCellValue('I2', 'Division');
    $objWorksheet->setCellValue('J2', 'Student ID');
    $objWorksheet->setCellValue('H2', 'User ID');
  ...
  # add the students
    ...
    foreach($module_details['students'] as $student_email => &$student_details) {
      ...
      # output the general information
        ...
        $objWorksheet->setCellValue('E'.$row, $student_details['role']);
        $objWorksheet->setCellValue('F'.$row, $student_details['hospital']);
        $objWorksheet->setCellValue('G'.$row, $student_details['other_company']);
        $objWorksheet->setCellValue('H'.$row, $student_details['department']);
        $objWorksheet->setCellValue('I'.$row, $student_details['division']);
        $objWorksheet->setCellValue('J'.$row, $student_details['employeeID']);
        $objWorksheet->setCellValue('H'.$row, $student_details['user_id']);
