...
# set defaults
  ...
  $filename = 'Report - Patient Safety and Pain Modules - ' . date('F j, Y') . '.xlsx';
...
# assemble the xls
  ...
  # add the column names
    ...
    $objWorksheet->setCellValue('B2', 'First Name');
    $objWorksheet->setCellValue('C2', 'Last Name');
    $objWorksheet->setCellValue('D2', 'Email');
    $objWorksheet->setCellValue('E2', 'Program');
    $objWorksheet->setCellValue('F2', 'Date of completion');
  ...
  # add the students
    ...
    foreach($module_details['students'] as $student_email => &$student_details) {
      ...
      # output the general information
        ...
        $objWorksheet->setCellValue('B'.$row, $student_details['first_name']);
        $objWorksheet->setCellValue('C'.$row, $student_details['last_name']);
        $objWorksheet->setCellValue('D'.$row, strtolower($student_email));
        $objWorksheet->setCellValue('E'.$row, $student_details['program']);

        $timestamp_completed = strtotime($student_details['date_completed']);
        $objWorksheet->setCellValue('F'.$row, PHPExcel_Shared_Date::PHPToExcel($timestamp_completed));
