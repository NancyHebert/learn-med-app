...
# set defaults
  ...
  $filename = $program_name . ' iLEARN-Peds Report - ' . date('F j, Y') . '.xlsx';
...
//  d($residents);
  ...
  # set the cell caching method
    ...
    $objWorksheet->setTitle($program_name);

  # add the title

    $objWorksheet->setCellValue('A1', 'iLEARN-PEDS FAMILY MEDICINE COMPLETION REPORT - Updated: ' . date("j F Y"));

  # add the column names

    $objWorksheet->setCellValue('A2', 'Block');
  ...
  # for each resident
    foreach($residents as $blockname => &$residents_in_block) {
      ...
      preg_match('/([1-9][0-9]*)$/', $blockname, $matches);
      $block_num = (isset($matches[1]))? $matches[1] : "n/a";

      foreach($residents_in_block as $email => &$details) {

        # output the general information

          $objWorksheet->setCellValue('A'.$row, $block_num);
