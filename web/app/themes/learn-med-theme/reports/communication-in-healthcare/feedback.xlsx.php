...
# get the list of modules to report on
  ...
  $filename = sprintf($config['reports']['feedback']['filename'], date('F j, Y'));
  ...
  foreach($modules as $module_activity_id => &$module_details) {
    ...
    # fetch the question's text
      ...
      foreach($module_details['questions']['feedback'] as $question_id => $question_details) {
      ...
      $module_details['questions']['feedback'] = $questions;
