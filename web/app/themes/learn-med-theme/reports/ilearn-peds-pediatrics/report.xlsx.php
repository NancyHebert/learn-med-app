...
# load the config
  ...
  $filename = $config["report_name"] . ' iLEARN-Peds Report - ' . date('F j, Y') . '.xlsx';
...
# check which resident never logged in
  ...
  foreach($residents as $year => &$residents_in_year) {
    foreach($residents_in_year as $email => &$details) {
...
//  d($residents);
  ...
  # for each resident
    foreach($residents as $yearname => &$residents_in_year) {
