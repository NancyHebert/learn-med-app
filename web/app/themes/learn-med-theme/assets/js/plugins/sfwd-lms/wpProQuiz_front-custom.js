	...
	$.wpProQuizFront = function(element, options) {
		...
		var reviewBox = new function() {
			...
			function scroll(index) {
				var $items = $(globalNames.questionList).children();
...
//					});
				...
				if ('data' in resultData[questionId] && !$.isEmptyObject(resultData[questionId]['data'])) {
					if (!('retries' in resultData[questionId])) {
						resultData[questionId]['retries'] = [];
					}
					resultData[questionId]['retries'].push(data);
				} else {
		...
		plugin.methode = {
			...
			startQuiz: function(loadData) {
				...
				$e.find(globalNames.listItems).each(function(i, v) {
					var $this = $(this);
...
//					}
				...
				$e.find('.wpProQuiz_sortable').parents('ul').sortable({
					update: function( event, ui ) {
						var $p = $(this).parents(globalNames.listItems);
...
//					plugin.methode.showQuizSummary();
			...
			showQuestionObject: function(obj) {
				if(!obj.length && bitOptions.forcingQuestionSolve && bitOptions.quizSummeryHide && bitOptions.reviewQustion) {
					for(var i = 0, c = $e.find(globalNames.listItems).length; i < c; i++) {
						if(!quizSolved[i]) {
...
//				$e.find('.wpProQuiz_cloze input').val('');
				...
				$e.find(globalNames.listItems).data('check', false);

				$e.find('.wpProQuiz_response').hide().children().hide();

				plugin.methode.resetMatrix($e.find(globalNames.listItems));
			...
			initQuiz: function() {
				...
				globalElements.next.click(function() {
					console.log('initQuiz: "Check Answer" clicked for question id ' + currentQuestion.index());
				...
				$e.find(globalNames.check).click(function() {
					console.log('initQuiz2: "Check Answer" clicked for question id ' + currentQuestion.index());
					if(bitOptions.forcingQuestionSolve && !quizSolved[currentQuestion.index()]
							&& (bitOptions.quizSummeryHide || !bitOptions.reviewQustion)) {
	...
	$.fn.wpProQuizFront = function(options) {
		return this.each(function() {
			if(undefined == $(this).data('wpProQuizFront')) {
				...
				console.log("wpProQuizFront custom version installed");
			}
		});
	};
